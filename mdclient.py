...
class MDClient(MDInterface):
    ...
    def connect(self):
        ...
        if self.sessionID:
            if DEBUG: print "Trying to resume session ", self.sessionID
            # Do reconnect
            if self.reqSSL:
...
#        print "buffer: >>>>>>>>>>>", self.buffer, "<<<<<<<<<<<<"
        ...
        if pos>-1:
            ...
            if not len(self.buffer) and not self.EOT:
                if self.__fetchData() < 0: return None
            return line
        if self.EOT:
            return None
        if self.__fetchData() <=0:
    ...
    def __fetchData(self):
        while 1:
            ...
            # Look for newline
            pos = self.buffer.find('\n')
            if pos>-1 and pos<len(self.buffer)-1: break
    ...
    def eot(self):
        if len(self.buffer): return 0
        if self.EOT: return 1
        if self.__fetchData() <=0: return 1
    ...
    def constraintAddNotNull(self,directory,attribute,name):
        command='constraint_add_not_null '+directory+" "+attribute+" "+name
        self.execute(command)

    def constraintAddUnique(self, directory, attribute, name):
        command='constraint_add_unique '+directory+" "+attribute+" "+name
        self.execute(command)

    def constraintAddReference(self, directory, attribute, reffered_attr, name):
        command='constraint_add_reference '+directory+' '+attribute+' '+reffered_attr
        command=command + ' ' + name
        self.execute(command)

    def constraintAddCheck(self, directory, check, name):
        command='constraint_add_check '+directory+' ' + self.__quoteValue(check)+' '+name
        self.execute(command)
